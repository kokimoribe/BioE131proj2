...
- **Objective**: Write a program that encodes realistic bacterial protein-coding genes determined by having it follow specific input and output criteria. See *Project_2_Description.pdf* and *InputOutputCriteriaOutline.txt* for a comprehensive overview.
...
1. Have the required input files in the same directory as *runproject.py*. See the *example input files* folder for reference.
2. Run *runproject.py* via command-line: ```python runproject.py```
3. Open *output.fasta* to view the results. (Located in the same directory as runproject.py)
